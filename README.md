...
원작(,'raylib를 이용해 만든 클래식 테트리스) 리포지토리를 기반으로 커스텀 디지털 신호처리(DSP) 오디오 합성 엔진 및 안정적인 오디오 자원 관리 아키텍처를 추가하여 포크(Fork) 및 고도화하였습니다.
...
// =============================================================중요============================================================= //
   실행 파일 이용 시: game.zip 의 압축을 풀어 실행할 수 있습니다. (Windows 스마트앱 컨트롤/SmartScreen 경고가 뜰 경우, game.exe 우클릭 ➔ [속성] ➔ 일반 탭 하단의 **[차단 해제]**를 체크하신 후 실행해 주세요.)

소스 코드 빌드 시: 실행이 원활하지 않을 경우, 포함된 소스 코드(main.cpp, tetray.cpp)를 Visual Studio 등 개발 환경에 추가하여 직접 빌드(F5)하시면 보안 경고 없이 정상 실행됩니다.
